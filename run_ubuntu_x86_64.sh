...
#export ARCH=x86_64
export INSTALL_PATH=$LROOT/rootfs_ubuntu_x86_64/boot
export INSTALL_MOD_PATH=$LROOT/rootfs_ubuntu_x86_64
export INSTALL_HDR_PATH=$LROOT/rootfs_ubuntu_x86_64/usr

kernel_build=$PWD/rootfs_ubuntu_x86_64/usr/src/linux
rootfs_path=$PWD/rootfs_ubuntu_x86_64
rootfs_image=$PWD/rootfs_ubuntu_x86_64.ext4
...
prepare_rootfs(){
		if [ ! -d $rootfs_path ]; then
			...
			tar -Jxf rootfs_ubuntu_x86_64.tar.xz
		fi
}
...
build_kernel_devel(){
	...
	rm rootfs_ubuntu_x86_64/lib/modules/$kernver/build
	...
	ln -s /usr/src/linux rootfs_ubuntu_x86_64/lib/modules/$kernver/build
...
check_root(){
		if [ "$(id -u)" != "0" ];then
			...
			echo "sudo ./run_ubuntu_x86_64.sh build_rootfs"
			exit 1
		fi
}
...
build_rootfs(){
		if [ ! -f $rootfs_image ]; then
			...
			mkfs.ext4 rootfs_ubuntu_x86_64.ext4
			mkdir -p tmpfs
			echo "copy data into rootfs..."
			mount -t ext4 rootfs_ubuntu_x86_64.ext4 tmpfs/ -o loop
			cp -af rootfs_ubuntu_x86_64/* tmpfs/
			umount tmpfs
			chmod 777 rootfs_ubuntu_x86_64.ext4
		fi
...
run_qemu_ubuntu(){
		qemu-system-x86_64 -m 1024 -enable-kvm\
			...
			-drive if=none,file=rootfs_ubuntu_x86_64.ext4,id=hd0 \
...
case $1 in
	...
	run)
		...
		if [ ! -f $LROOT/arch/x86/boot/bzImage ]; then
			...
			echo "./run_ubuntu_x86_64.sh build_kernel"
			exit 1
		fi
